...
registerTool({
  name: "run_command",
  description:
    "Выполнить команду в терминале на устройстве пользователя (встроенный Linux: Alpine rootfs — bash, coreutils, python3, pip, git, curl " +
    "и стандартные утилиты; python3 уже установлен). Вывод команды вернётся как результат. Не запускай интерактивные программы " +
    "(vim, nano, top) — только однократные команды. Пакеты ставь штатным менеджером среды: в основном режиме Linux это `apk add <пакет>` " +
    "(Alpine, root-права есть; обновление списков — `apk update`), в запасном Termux-режиме — `apt install <пакет>` (sudo не нужен). " +
    "Сначала проверь доступный менеджер: `command -v apk || command -v apt`. " +
    "Python-пакеты — `pip install <пакет>` (при отсутствии pip: `apk add py3-pip`). " +
