...
registerTool({
  name: "run_command",
  description:
    "Выполнить команду в терминале на устройстве пользователя (полноценный Linux: bash, python3, pip, git, apk " +
    "и стандартные утилиты). Вывод команды вернётся как результат. Не запускай интерактивные программы " +
    "(vim, nano, top) — только однократные команды. Пакеты ставь через apk add (sudo не нужен). " +
