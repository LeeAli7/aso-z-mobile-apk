...
Команды выполняются во встроенном Linux-рантайме приложения (как Termux: bash, python3, pip, apt, git). Пакеты ставь через apt install. Выполняй команды по одной, дожидайся вывода. Не запускай интерактивные процессы (vim, nano, top) без --non-interactive.
