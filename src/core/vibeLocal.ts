...
Команды выполняются во встроенном Linux-рантайме приложения (Alpine rootfs: bash, python3, pip, git, apk). Пакеты ставь через apk add. Выполняй команды по одной, дожидайся вывода. Не запускай интерактивные процессы (vim, nano, top) без --non-interactive.
