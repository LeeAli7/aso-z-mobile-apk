...
Команды выполняются во встроенном Linux-рантайме приложения (проot/Alpine: bash, python3, pip, apk, git — или запасной Termux-режим с apt). Пакеты ставь через apk add (основной режим) или apt install (Termux-режим); сначала проверь which apk apt. Выполняй команды по одной, дожидайся вывода. Не запускай интерактивные процессы (vim, nano, top) без --non-interactive.
