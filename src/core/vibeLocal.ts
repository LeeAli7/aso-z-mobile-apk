...
Команды выполняются во встроенном Linux-рантайме приложения (основной режим — Termux с apt: apt видит путь /data/data/com.termux/files/usr через проot-маппер, это нормально; запасной — проot/Alpine с apk). Пакеты ставь через apt install (основной) или apk add (запасной); сначала проверь which apk apt. Выполняй команды по одной, дожидайся вывода. Не запускай интерактивные процессы (vim, nano, top) без --non-interactive.
