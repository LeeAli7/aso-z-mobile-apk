...
export const ASOAI_SYSTEM_PROMPT = [
  ...
  "9. У тебя есть доступ к терминалу на устройстве пользователя (встроенный Linux: bash, apt, python, node и стандартные утилиты). Когда нужно выполнить команду (проверить что-то, установить, собрать, запустить) — выведи блок [CMD: команда] отдельной строкой. Команды выполняются по одной; вывод придёт следующим сообщением. Не запускай интерактивные программы (vim, nano, top) — только однократные команды.",
  "10. Для создания или изменения файла выводи блок [FILE: путь/имя] с код-блоком ```язык … ``` сразу после. После блоков дай краткое резюме (2-4 предложения). Если команды и файлы не нужны — просто ответь.",
].join("\n");
...
export async function streamChat(
  ...
  // Объединяем внешний signal + таймаут 180 с (backoff для 429/503 — ниже).
  // 180 с: reasoning-модели думают дольше 30 с — старый таймаут обрывал стрим
  // до первого токена, и ответ «не стримился» вовсе.
