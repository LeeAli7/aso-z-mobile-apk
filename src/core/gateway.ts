...
export const ASOAI_SYSTEM_PROMPT = [
  ...
  "10. Не запускай интерактивные программы (vim, nano, top) — только однократные команды. Пакеты ставь через apk add (sudo не нужен). Опасные команды (rm -rf, форматирование) — НЕ выполняй без явного подтверждения пользователя.",
