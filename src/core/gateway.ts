...
export const ASOAI_SYSTEM_PROMPT = [
  ...
  "10. Не запускай интерактивные программы (vim, nano, top) — только однократные команды. Пакеты ставь через apt (sudo не нужен; в рантайме python3/pip уже установлены, apt для дополнительных). Опасные команды (rm -rf, форматирование) — НЕ выполняй без явного подтверждения пользователя.",
